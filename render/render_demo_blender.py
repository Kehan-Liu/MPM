...
def render_demo(demo_name, start_frame=0, end_frame=None, skip_existing=False):
    ...
    for ply_file in ply_files:
        ...
        if not os.path.isdir(rigid_dir):
            print(f"Warning: Missing rigid dir: {rigid_dir}")
        ...
        # Command
        # blender ./assets/assets.blend -b -P render/render_frame_blender.py -- <ply> <rigid_dir> <frame_str> <out>
        cmd = [
            "blender",
            os.path.join("assets", "assets.blend"),
            "-b",
            "-P",
            os.path.join("render", "render_frame_blender.py"),
            "--",
            ply_file,
            rigid_dir,
            frame_str,
